...
export default function AdminLocationMap({ latitude, longitude, editable, onChange }) {
  ...
  return (
    <div className="admin-map-shell">
      {Number.isFinite(latitude) && Number.isFinite(longitude) ? (
        <>
          <div ref={containerRef} className="admin-map" />
          <div className="admin-map-hint">
            {editable ? 'Click the map or drag the marker to adjust.' : 'Current saved location.'}
          </div>
        </>
      ) : (
        <div className="admin-map-empty">Enter latitude and longitude to show this image on the map.</div>
      )}
    </div>
  );
}
